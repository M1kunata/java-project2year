...
public class main extends readdata {
    ...
    public void working() {
        ...
        for (int i = 0; i < config.getsupplierNum(); i++) {
            supplier_thread sup = new supplier_thread("SupplierThread_" + i);
            sup.setaffersup(afterSupplierPut);
        ...
        for (int i = 0; i < config.getfactoryNum(); i++) {
            ...
            fac.setafterday(afterDayPrint);
            fac.setafterfaccount(afterFactoryCountShip);
            fac.setafterfaccountlef(afterFactoryCountLeft);
            fac.setafterfacget(afterFactoryGet);
            fac.setafterfacship(afterFactoryShip);
            fac.setaftersupplier(afterSupplierPut);
            fac.setdays(config.getdays());
            fac.setfrieght(allf);
            fac.setware(allware);
            fac.setmaxPro(config.getfactoryMax());
            allfac.add(fac);
        }
